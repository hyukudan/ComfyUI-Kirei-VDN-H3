...
class VDNState:
    ...
    Deliberately not an nn.Module: branch weights live in their own additional
    ModelPatcher, so attaching this state to diffusion_model cannot accidentally make
    Comfy move/account the same 4+ GiB twice.
    """
...
def _normalise_and_rope(attn: Any, q: torch.Tensor, k: torch.Tensor, rope_freqs):
    ...
    if getattr(model_management, "in_training", False) or torch.is_grad_enabled():
        ...
        # The raw video/text slices needed by the linear branch have already been
        # compact-copied, so the fused QKV backing storage can be safely mutated here.
    ...
    def vdn_forward(x, rope_freqs=None, transformer_options=None):
        ...
        try:
            ...
            if linear_active:
                ...
                # Compact copies intentionally break the shared fused-QKV storage so it
                # can die before the recurrent branch starts.
            ...
            # These full-sequence tensors can be multiple GiB and are dead now.  The
            # linear branch retains only compact raw video/text copies.
