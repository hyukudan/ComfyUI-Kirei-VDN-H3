...
def _window_geometry(num_frames: int, radius: int, chunk: int) -> tuple[tuple[tuple[int, int], ...], bool]:
    try:
        ...
        def window_bounds(n: int, r: int, c: int = 0):
            ...
            return [(((t // c) - r) * c, ((t // c) + r + 1) * c - 1) for t in range(n)]
    ...
    if len(bounds) != num_frames:
        raise ValueError(f"window implementation returned {len(bounds)} bounds for {num_frames} frames")
...
def from_comfy_packed_layout(
    ...
    *,
    ...
    if anchor_frames not in _ANCHOR_MODES:
        raise ValueError(f"anchor_frames must be one of {sorted(_ANCHOR_MODES)}, got {anchor_frames!r}")
    try:
    ...
    if len(signature) != 5:
        raise ValueError(f"PackedLayout.signature must be (text_len, latent_t, latent_h, latent_w, audio_t), got {signature!r}")
    ...
    if text_end - text_start != text_len:
        raise ValueError(f"text segment length disagrees with PackedLayout.signature: {text_end - text_start} != {text_len}")
    ...
    if video_rows % latent_t:
        raise ValueError(f"{video_rows} video rows are not divisible by {latent_t} latent frames")
    tokens_per_frame = video_rows // latent_t
    frame_size = ((latent_h + 1) // 2, (latent_w + 1) // 2)
    if frame_size[0] * frame_size[1] != tokens_per_frame:
        raise ValueError(
            f"patched grid {frame_size[0]}x{frame_size[1]} does not match {tokens_per_frame} video tokens per frame"
        )
...
def layout_from_payload(payload: dict[str, Any] | None, x: Any, context: Any, config: dict[str, Any]) -> VDNLayout:
    ...
    try:
        ...
        raise TypeError(
            "VDN-H3 expected x=(video[B,C,T,H,W], audio[...,T]) and context[B,text,hidden]"
        ) from exc
    ...
    if packed is None or tuple(getattr(packed, "signature", ())) != signature:
        try:
            ...
            raise RuntimeError(
                "minimax_payload has no layout matching this input and ComfyUI's MiniMax-H3 PackedLayout is unavailable"
            ) from exc
        packed = PackedLayout(
            *signature,
...
def with_window(layout: VDNLayout, *, radius: int, chunk: int) -> VDNLayout:
    """Return a layout with recomputed window metadata."""
