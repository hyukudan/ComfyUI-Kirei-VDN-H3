"""ComfyUI nodes for the optimized Kirei VDN-H3 integration."""
...
def _auto_branch_mode(model, branch_bytes: int) -> str:
    ...
    try:
        ...
        return "stream"
    # Conservative by design: a 24 GiB card should normally stream, whereas 48/96 GiB
    # cards can keep the ~4.3 GiB branch resident without starving H3 activations.
...
def apply_checkpoint(
    ...
    *,
    ...
    """Load, validate and apply the complete optimized VDN stack transactionally."""
    ...
    try:
        for name in wanted:
            ...
            merged = 0
            # Apply one at a time: default and turbo may legitimately target the same
            # module and ModelPatcher should retain both low-rank terms.
            for term in merge:
        ...
        bypass_count, lora_runtime = install_bypass(cloned, bypass_terms)
...
class KireiApplyVDNH3:
    ...
    DESCRIPTION = "Apply optimized VideoDeltaNet H3 with automatic memory, LoRA and attention dispatch."
...
class KireiApplyVDNH3Alpha:
    """Legacy node id/schema retained so saved alpha workflows still open."""
    ...
    DESCRIPTION = "Legacy Kirei VDN-H3 Alpha workflow compatibility node."
...
class KireiReleaseVDNH3Weights:
    ...
    DESCRIPTION = "Explicitly release this model's VDN caches and streamed/resident auxiliary weights."

    def release(self, model):
        state = getattr(model, "object_patches", {}).get("diffusion_model._vdn_h3_private_state")
...
NODE_DISPLAY_NAME_MAPPINGS = {
    ...
    "KireiApplyVDNH3Alpha": "Kirei Apply VDN-H3 (Legacy Alpha)",
